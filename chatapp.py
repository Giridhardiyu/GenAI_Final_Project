...
import os
os.environ["GOOGLE_API_KEY"] = "AIzaSyBRqwSuzqeyzROpP7ro5S_3Iy2ROwtsS5c"
...
def cached_similarity_search(query, vector_store):
    return vector_store.similarity_search(query, k=3)  # Limit the number of docs


def get_vector_store(text_chunks, save=True):
...
def user_input(user_question):
    ...
    docs = cached_similarity_search(user_question, new_db)  # Use cached search function
    ...
    with st.sidebar:
        ...
        if st.button("Submit & Process"):
            with st.spinner("Processing..."):
                ...
                get_vector_store(text_chunks)  # Save vector store only once
                st.success("Done")
